...
public class SimpleWeightedRandomGenerator<E, T> implements WeightedRandomGenerator<T> {
    ...
    public static <T> @NotNull WeightedRandomBuilder<T> builder(@NotNull Map<T, Double> map) {
    ...
    private static final class Builder<E, T> implements WeightedRandomBuilder<T> {
        ...
        @Override
        public @NotNull WeightedRandomGenerator<T> build() {
            if (collection.isEmpty()) {
                throw new IllegalStateException("Collection is empty");
            }
            ...
            return new SimpleWeightedRandomGenerator<>(sum, collection, objectFunction, weightFunction, defaultValue);
        }
    }
}
